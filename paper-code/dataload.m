...
function [inflow, params, sysparams] = dataload()
    ...
    params(1) = struct( ...
        'season', 'wet', 'center_day', 157, 'inflow_mean', 2.58e7, ...
        'inflow_std', 9.21e6, 'outflow_mean', 2.37e7, 'outflow_std', 9.12e6, ...
        'constant', -0.0018, 'coef1', 0.836, 'coef2', 0.165, 'resid_var', 0.0959, ...
        'AR_const', -0.0004, 'AR_coef', 0.995, 'AR_resid_var', 0.1042, ...
        'omega', 0.172, 'alpha', 0.815, 'gamma', 0.00832, 'error_mean', 0.069, ...
        'error_std', 0.0364);

    params(2) = struct( ...
        'season', 'dry', 'center_day', 275, 'inflow_mean', 1.06e7, ...
        'inflow_std', 2.47e6, 'outflow_mean', 9.69e6, 'outflow_std', 2.79e6, ...
        'constant', -0.0013, 'coef1', 0.830, 'coef2', 0.169, 'resid_var', 0.295, ...
        'AR_const', 0.0020, 'AR_coef', 0.950, 'AR_resid_var', 0.3188, ...
        'omega', 0.179, 'alpha', 0.796, 'gamma', 0.0404, 'error_mean', 0.21, ...
        'error_std', 0.11);

    % Unit 1: Bonneville
    sysparams(1) = struct( ...
        'unit', 1, 'a', 0.0026, 'b', 0.4404, ...
        'min_ut', 3.12e6, 'max_ut', 1.37e7, ...
        'min_h', 21.33, 'max_h', 23.49, ...
        'min_V', 7.72e8, 'max_V', 9.56e8, ...
        'RR_dn', -642225, 'RR_up', 611643, ...
        'F', 1154, 'SOC', 0.5);

    % Unit 2: Dalles
    sysparams(2) = struct( ...
        'unit', 2, 'a', 4.1, 'b', 0.134, ...
        'min_ut', 5.27e6, 'max_ut', 1.65e7, ...
        'min_h', 47.24, 'max_h', 50.29, ...
        'min_V', 8.36e7, 'max_V', 1.33e8, ...
        'RR_dn', -499508, 'RR_up', 479120, ...
        'F', 1780, 'SOC', 0.5);
