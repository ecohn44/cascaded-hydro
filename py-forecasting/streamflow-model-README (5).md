...
$$
V_{i,t}^{\mathrm{norm}}=\operatorname{clip}\left(\frac{H_{i,t}^{\mathrm{norm}}-b_i}{a_i},0,1\right).
$$
...
$$
b_i^{q}=\frac{1}{N_i}\sum_t\left(q_{i,t}^{\mathrm{recon}}-q_{i,t}^{\mathrm{reported}}\right),
$$
...
$$
q_{i,t}^{\mathrm{avg}}=\operatorname{median}\left\{q_{i,t-k}^{\mathrm{bc}}:k=0,\ldots,5\right\}.
$$
...
$$
\mathcal L_{ji}=\left\{\left\lceil\frac{d_{ji}}{v_{\max}}\right\rceil,\ldots,\left\lfloor\frac{d_{ji}}{v_{\min}}\right\rfloor\right\}.
$$

The selected lag maximizes the sample correlation between upstream outflow and downstream reconstructed inflow:

$$
h_{ji}^{*}=\underset{h\in\mathcal L_{ji}}{\operatorname{argmax}}\ \operatorname{Corr}\left(u_{j,t-h},q_{i,t}^{\mathrm{avg}}\right).
$$
...
$$
\widehat{\boldsymbol\theta}_i=\underset{\boldsymbol\theta_i}{\operatorname{argmin}}\left\{\lVert\mathbf y_i-\mathbf X_i\boldsymbol\theta_i\rVert_2^2+\lambda_i\lVert\boldsymbol\theta_i\rVert_2^2\right\}.
$$
...
$$
\sigma_{i,t}^2=\omega_i+\alpha_i\varepsilon_{i,t-1}^2+\beta_i\sigma_{i,t-1}^2+\gamma_{i,u}\widetilde u_{j,t-h_i}+\gamma_{i,r}\left|\Delta\widetilde u_{j,t-h_i}\right|.
$$
...
$$
\left|\Delta\widetilde u_{j,t}\right|=\frac{|u_{j,t}-u_{j,t-1}|-r_j^{\min}}{r_j^{\max}-r_j^{\min}}.
$$
...
$$
\ell_{i,t}=\log\Gamma\left(\frac{\nu_i+1}{2}\right)-\log\Gamma\left(\frac{\nu_i}{2}\right)-\frac{1}{2}\log\left[\pi(\nu_i-2)\right]-\frac{1}{2}\log\sigma_{i,t}^2-\frac{\nu_i+1}{2}\log\left(1+\frac{\varepsilon_{i,t}^2}{(\nu_i-2)\sigma_{i,t}^2}\right).
$$
...
$$
\widehat C_{i,p}=\frac{1}{N_i}\sum_t\mathbf 1\left\{|\varepsilon_{i,t}|\leq c_{p,\nu_i}\sigma_{i,t}\right\}.
$$
...
$$
R=\operatorname{Corr}(\mathbf z_t)=
\begin{bmatrix}
1 & 0.0400 & 0.0194\\
0.0400 & 1 & 0.0785\\
0.0194 & 0.0785 & 1
...
$$
D_t=\operatorname{diag}(\sigma_{\mathrm{JDA},t},\sigma_{\mathrm{TDA},t},\sigma_{\mathrm{BON},t}).
$$
