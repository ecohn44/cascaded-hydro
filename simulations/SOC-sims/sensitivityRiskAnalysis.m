...
X_sweep = cell(length(alpha_set), length(baseline_set)); 
q_sweep = cell(length(alpha_set), length(baseline_set)); 
...
for a = 1:length(alpha_set)
    ...
    for d = 1:length(baseline_set)
        ...
        % Save inflow for historical replay
        q_sweep{a, d} = q(1+lag:T+lag, :);
        ...
        try 
            ...
            % Save policy trajectory for historical replay
            X_sweep{a, d} = X;
