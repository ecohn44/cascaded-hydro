...
function [params, sysparams, scenarioparams] = dataload(n, N)
    ...
    scenarioparams(1).amp1         = 0.85;      % +% gain for pulse 1
    scenarioparams(1).amp2         = 1.5;      % +% gain for pulse 2
    scenarioparams(1).w1           = 12;        % first drought pulse lasts 8 hours
    scenarioparams(1).w2           = 16;        % second drought pulse lasts 4 hours
    scenarioparams(1).t0           = [0.3, 0.8];  % pulses at 30% and 50% of horizon
    ...
    scenarioparams(2).amp1         = 0.15;    % Magnitude of drought event %.4
    scenarioparams(2).nEvents      = 1;      % number of events
    scenarioparams(2).daysPerEvent = 0.8;    % drought length %1.2
    scenarioparams(2).tauHours     = 12;     % decay rate
    scenarioparams(2).unitDelay    = 16;     % time units between drought events 
    scenarioparams(2).startSteps   = 8;      % time before initial drought event begins 
    scenarioparams(2).recoverHours = 8;     % recovery time from drought event to q0
