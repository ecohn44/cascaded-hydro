...
% ------------------------------------------------------------------------
% Aggregators across units
% ------------------------------------------------------------------------
P_M1_allUnits = [];   % total power over all units (time x 1)
P_M2_allUnits = [];
P_M3_allUnits = [];

% For combined std-dev and bounds figures
std_M2_all  = [];
std_M3_all  = [];
Vmin_M1_all = [];
Vmax_M1_all = [];
...
for uu = unitIdx(:)'   % uu is the actual unit number in the files
    ...
    % --------------------------------------------------------------------
    % DATA FOR SUPER STANDARD-DEVIATION PLOTS
    % --------------------------------------------------------------------
    std_M2_all(:,idx) = M2.std_hat(:,uu);
    std_M3_all(:,idx) = M3.std_hat(:,uu);

    % --------------------------------------------------------------------
    % DATA FOR SUPER SOC-BOUND PLOTS
    % --------------------------------------------------------------------
    ...
    % --------------------------------------------------------------------
    % POWER: accumulate only (no per-unit plotting)
    % --------------------------------------------------------------------
    p_M1 = M1.X(:,colP);
    p_M2 = M2.X(:,colP);
    p_M3 = M3.X(:,colP);
...
% ------------------------------------------------------------------------
% AFTER LOOP: Combined "super" figure for bounds + std (4 x 2)
% ------------------------------------------------------------------------
...
for i = 1:n_units
    ...
    % -------- LEFT COLUMN: SoC bounds + trajectories --------
    nexttile; hold on; grid on;
    
    % M1
    p1a = plot(tt_all, Vmin_M1_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', blueBound);
    p1b = plot(tt_all, Vmax_M1_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', blueBound, 'HandleVisibility','off');
    h1  = plot(tt_all, V_M1_all(:,i),    'LineWidth',2, 'Color', 0.6*blueBound);
    
    % M2
    p2a = plot(tt_all, Vmin_M2_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', greenBound);
    p2b = plot(tt_all, Vmax_M2_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', greenBound, 'HandleVisibility','off');
    h2  = plot(tt_all, V_M2_all(:,i),    'LineWidth',2, 'Color', 0.6*greenBound);
    
    % M3
    p3a = plot(tt_all, Vmin_M3_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', redBound);
    p3b = plot(tt_all, Vmax_M3_all(:,i), 'LineWidth',3, 'LineStyle','-.', 'Color', redBound, 'HandleVisibility','off');
    ...
    xlim([1, T])
    ylim([0 0.02]);
    title(sprintf('Unit %d SoC Lower Bounds', unitIdx(i)), 'FontSize', fontTitle);
    xlabel('Time (h)');  
    ylabel('SoC');
    set(gca, 'FontSize', fontAxes);

    % -------- RIGHT COLUMN: Std deviation (DET/DIU/DDU) --------
    nexttile; hold on; grid on;

    % DET: zero line
    h1s = plot(tt_all, zeros(size(tt_all)), 'k-', 'LineWidth', 2);
    % DIU
    h2s = plot(tt_all, std_M2_all(:,i), 'LineWidth', 2, 'Color', greenBound);
    % DDU
    h3s = plot(tt_all, std_M3_all(:,i), 'LineWidth', 2, 'Color', redBound);
...
lgStd = legend([hDET_std hM2_std hM3_std], ...
    {'DET', 'DIU', 'DDU'}, ...
    'Orientation','horizontal', 'FontSize', fontLegend, 'Box','on');
drawnow;
posS = lgStd.Position;
