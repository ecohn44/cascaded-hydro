function [model, obj, X, std_hat, V_eff] = baseOptimization(T, N, c, q, lag, scale, framework, bounds, params, s)
    ...
    for t = 1:T
        ...
        if t == 1 % Initial conditions
            ...
            cons = [cons, s(1).min_V + V1_min_shift <= s(1).V0 + q1 - (u1 + s1) <= s(1).max_V + V1_max_shift];
            cons = [cons, s(2).min_V + V2_min_shift <= s(2).V0 + q2 - (u2 + s2) <= s(2).max_V + V2_max_shift];
            ...
            % Volume Bounds (with chance constrained shift)
            cons = [cons, s(1).min_V + V1_min_shift <= X(t-1,1) + q1 - (u1 + s1) <= s(1).max_V + V1_max_shift];
            cons = [cons, s(2).min_V + V2_min_shift <= X(t-1,6) + q2 - (u2 + s2) <= s(2).max_V + V2_max_shift];
