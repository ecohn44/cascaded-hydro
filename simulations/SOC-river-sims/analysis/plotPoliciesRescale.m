...
for i = 1:n_units
    ...
    ax_inset = axes('Position', [ ...
        pos(1) + 0.1*pos(3), ...   % shift right
        pos(2) + 0.25*pos(4), ...   % shift up
        0.4*pos(3), ...            % width
        0.35*pos(4)]);              % height
    ...
    % Plot same data
    plot(u1_phys, 'Color', c1, 'LineWidth', 1.5);
    plot(u2_phys, 'Color', c2, 'LineWidth', 1.5);
    plot(u3_phys, 'Color', c3, 'LineWidth', 1.5);
    
    % Zoom region     
    ylim(ax_inset, [5.5 8.9])  

    if i == 1
        xlim(ax_inset, [33 43])
        xticks(ax_inset, [35, 40])
    elseif i ==2
        xlim(ax_inset, [35 45])
    else 
        xlim(ax_inset, [40 50])
    ...
    if i == 1
        title(ax, 'Release (10^3 m^3/s)','FontSize',font_title,'FontWeight','normal');
    elseif i == n_units
        xlabel(ax, 'Time (hours)','FontSize',font_axis);
    end
    ylabel(ax, sprintf('Unit %d', sp.unit),'FontSize',font_unit);

    xlim(ax, [1, T]);
    ylim(ax, [1, 9]);
    yticks(ax, u_ticks);

    set(ax,'FontSize',12,'LineWidth',1,'Box','on','TickDir','out')
    ...
    elseif plot_storage_as == "volume"
        ...
        if i == 1
            title(ax, 'Reservoir Volume (10^6 m^3)', ...
