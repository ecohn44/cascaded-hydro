...
kappa = 1; %0.5:0.5:2;           % Forecast error
frameworks = ["diu", "ddu"];   % Uncertainty representation 
thetas = [0, 5, 10, 15];   %0:1:10;    % Real time tracking coefficient 
...
for y = 1:Y
    ...
    if test_mode
        train_years = 1:length(training_years);  % 2018–2023
    else
        train_years = setdiff(1:length(training_years),y);
    end
