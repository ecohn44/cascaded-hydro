...
results.V     = nan(n_units,T,M,K,S);
results.p     = nan(n_units,T,M,K,S);
results.u     = nan(n_units,T,M,K,S);
results.sp    = nan(n_units,T,M,K,S);
results.q     = nan(n_units,T,M,K,S);
results.std   = nan(n_units,T,M,K,S);

% Track infeasible outcomes 
failed = false(M, K, S);
failure_time = nan(M, K, S);
IVI = zeros(M,K,S);
        ...
        for s = 1:S
            ...
            for t = 1:T
                ...
                if ismember(result.status, {'OPTIMAL','SUBOPTIMAL','TIME_LIMIT'}) && isfield(result, 'x') && ~isempty(result.x) 
                    ...
                    q_mean(:,t) = X_t(:,5);
                    q_real(:,t) = q_mean(:,t) + kappa(k)*sigma_ddu(:).*Z(:,t,s);
                ...
                % Calculate IVI outside operating envelope
                env_error = max(SOC_p10(:,t)-V_history(:,t),0) + max(V_history(:,t)-SOC_p90(:,t),0);
                IVI(m,k,s) = IVI(m,k,s) + sum(env_error);
            ...
            results.V(:,:,m,k,s)   = V_history;
            results.p(:,:,m,k,s)   = p_history;
            results.u(:,:,m,k,s)   = u_history;
